// FIFA World Cup 2026 Regulations, Annex C (options 1-495).
// Each 8-letter value maps, in order, to winners A, B, D, E, G, I, K and L.
// Options are ordered by reverse lexicographic combinations of 8 groups from A-L.
...
const WINNER_GROUPS = ['A', 'B', 'D', 'E', 'G', 'I', 'K', 'L'] as const;
...
const qualifyingGroupCombinations = combinations('ABCDEFGHIJKL'.split(''), 8).reverse();

if (ALLOCATION_VALUES.length !== qualifyingGroupCombinations.length) {
  throw new Error('Invalid FIFA Annex C allocation table');
}

export const THIRD_PLACE_ALLOCATION_TABLE: Record<string, Record<string, string>> =
  Object.fromEntries(qualifyingGroupCombinations.map((groups, optionIndex) => [
    groups.join(''),
    Object.fromEntries(WINNER_GROUPS.map((winner, slotIndex) => [
      winner,
      ALLOCATION_VALUES[optionIndex]![slotIndex]!
    ]))
  ]));
